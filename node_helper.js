...
module.exports = NodeHelper.create({
    ...
    for (let curDsIdx = 0; curDsIdx < self.config.ds.length; curDsIdx++) {
      ...
      if ((this.config.provideDummyUrlAfterIterations < 1) || (this.iterationCnt < this.config.provideDummyUrlAfterIterations)){
        ...
        for (let i = 0; i < curDs.cams.length; i++) {
          let curCam = curDs.cams[i]
          console.log(
            self.name + ": Sending dummy url for cam "+i+" of DS with idx: " + curDsIdx
          );
          if (typeof curCam.name !== "undefined"){
  ...
  goPosition: async function (dsIdx, camName, position) {
    ...
    if (
      typeof self.ds[dsIdx] !== "undefined" &&
      ...
      if (camId) {
        if (
          typeof self.ds[dsIdx].ptzPresetInfo !== "undefined" &&
          ...
          if (
            ...
            var curRealPosition =
              self.ds[dsIdx].ptzPresetInfo[camId][position]["position"];
            console.log(
              self.name +
                ": New position with idx: " +
                position +
                " is " +
                curRealPosition
            );
          ...
          console.log(
            self.name +
              ": Could not change position of cam: " +
              camName +
              " of ds: " +
              dsIdx +
              " because no ptz preset info is available!"
          );
        }
      } else {
        console.log(
          self.name +
            ": Could not change postion of cam: " +
            camName +
            " because no id of this cam is known!"
        );
      }
    } else {
      console.log(
        self.name +
          ": Could not change position of cam: " +
          camName +
          " because no id name mapping is present for this ds (" +
          dsIdx +
          ") at the moment."
      );
    }
